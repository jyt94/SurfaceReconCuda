...
//extern const int VERTEX_OFFSETS[8][3];
//extern const int EDGE_VERTEX[12][2];

void MarchingCubeCUDA::Marching() {
	cint3 cellDim = surfaceGrid->cellResolution;
	int xWidth = surfaceGrid->vertexResolution.x;
	int xyWidth = surfaceGrid->vertexResolution.x * surfaceGrid->vertexResolution.y;

	float value[8];
	auto& sVertices = *(surfaceGrid->surfaceVertices);
	...
	for(auto const& vertex : sVertices){
		...
		if (EDGE_TABLE[tableIndex] != 0) {
			...
			if(GetSurfaceIndex(coord+cint3(1,0,0))==-1){
			...
			if (GetSurfaceIndex(coord+cint3(0, 1, 0))==-1){
			...
			if (GetSurfaceIndex(coord+cint3(0, 0, 1))==-1){
...
void MarchingCubeCUDA::InsertVertex(cint3 coord, int edgeNumber, float* value) {
...
int MarchingCubeCUDA::GetEdgeIndex(cint3 coord, int edgeNumber) {
	switch (edgeNumber) {
	case 0:
...
IdPoint MarchingCubeCUDA::CalculateIntersection(
	cint3 coord, 
	int edgeNumber,
	float* values
) {
	...
	x1 = GetPosition(coord, surfaceGrid->xmin, surfaceGrid->cellWidth);
	x2 = x1;
...
void MarchingCubeCUDA::Reindex(PointIdMapping& vertexMapping, vector<Triangle>& triangles) {
